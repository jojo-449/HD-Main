...
const BookingModal = ({ model, onClose }) => {
  ...
  // States for handling multiple images and loading
  const [isInstaLoading, setIsInstaLoading] = useState(false);
  const [isWaLoading, setIsWaLoading] = useState(false);
  const [currentImgIndex, setCurrentImgIndex] = useState(0);
  const [showCopySuccess, setShowCopySuccess] = useState(false);

  // 1. EXTRACT IMAGES: Use database images array, or fallback to a placeholder
  const images = Array.isArray(model.images) && model.images.length > 0 
    ? model.images 
    : ['https://via.placeholder.com/400x600?text=No+Portfolio+Images'];

  // 2. AUTO-SLIDE LOGIC: Changes image every 4 seconds if multiple images exist
  useEffect(() => {
    if (images.length > 1) {
      const interval = setInterval(() => {
        setCurrentImgIndex((prev) => (prev === images.length - 1 ? 0 : prev + 1));
  ...
  const handleInstagramBook = async () => {
    ...
    try {
      ...
      if (user?.token) {
        await api.post('/api/bookings', { 
          modelName: model.name, shootType: shootSummary, 
          date: formData.date, time: formData.time, requirements: dbRequirements 
        }, { headers: { Authorization: `Bearer ${user.token}` } });
      }
      setIsInstaLoading(false);
      setShowCopySuccess(true); 
      ...
      <div className="modal-container" onClick={e => e.stopPropagation()}>
        ...
        {showCopySuccess && (
          <div className="copy-instruction-overlay">
            <div className="instruction-card">
              ...
              <button className="btn-continue" onClick={() => window.location.href = 'https://www.instagram.com/_hdmodels'}>
                Open Instagram App
              </button>
              <button className="btn-cancel-link" onClick={() => setShowCopySuccess(false)}>Go Back</button>
            </div>
          </div>
        )}
        ...
        <div className="modal-grid">
          ...
          <div className="modal-col-stats">
            ...
            <div className="model-about-box">
              <h3>About {model.name}</h3>
              {/* Uses 'about' from database or 'bio' from Registry */}
              <p>{model.about || model.bio || "No biography provided."}</p>
            </div>
            <div className="stats-table">
              <div className="stat-row"><span>Height:</span> <strong>{model.height}</strong></div>
              <div className="stat-row"><span>Size:</span> <strong>{model.size}</strong></div>
                  ...
                  <div key={key} className={`shoot-card-alt ${item.selected ? 'active-card' : ''}`}>
                    ...
                    {item.selected && (
                      <div className="card-details-reveal">
                        ...
                        <input type="text" placeholder="Requirements..." value={item.specificReq} onChange={(e) => handleDetailChange(key, 'specificReq', e.target.value)} className="req-input-alt" />
                      </div>
                    )}
                  </div>
                );
